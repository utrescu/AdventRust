...
/// Aprofito la majoria del codi del primer problema perquè ara el tinc al cap
/// i no tinc ganes de pensar-lo diferent :-)
///
/// O sigui aquest donarà la solució dels dos problemes
///
fn main() {
    ...
    for rectangle in rectangles.iter() {
        ...
        for fila in rectangle.y..rectangle.y + rectangle.height {
            for columna in rectangle.x..rectangle.x + rectangle.width {
                let valor = match ocupacio.get(&(columna, fila)) {
                    Some(&o) => o,
                    None => 0,
                };
                items = items + 1;
